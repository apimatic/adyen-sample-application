...
const { HmacValidator } = require('adyen-checkout-apilib');
const {Client, PaymentsController} = require('adyen-checkout-apilib');
...
const client = new Client({
  timeout: 0,
  basicAuthUserName: process.env.ADYEN_BASIC_AUTH_USERNAME,
  basicAuthPassword: process.env.ADYEN_BASIC_AUTH_PASSWORD,
});
...
app.post("/api/sessions", async (req, res) => {
  ...
  try {
    ...
    const {result }  = await paymentsController.postSessions(undefined, {
